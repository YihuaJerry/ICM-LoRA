...
# 执行 t-SNE 降维
perplexity = min(5, all_vectors.shape[0] - 1)  # 确保 perplexity 合理
tsne = TSNE(n_components=2, random_state=42, perplexity=perplexity)
tsne_results = tsne.fit_transform(all_vectors)

# 绘制 t-SNE 图
plt.figure(figsize=(10, 8))
for label, color in zip(["dog", "sofa", "cat","bicycle","motorbike"], ["blue", "green", "red","black","yellow"]):
    indices = all_labels == label
    plt.scatter(tsne_results[indices, 0], tsne_results[indices, 1], 
                label=label, alpha=0.7, edgecolors='k', s=50)

plt.title(f"t-SNE Plot of {keyword.capitalize()} Vectors")
plt.legend()
plt.savefig(f"tsne_all_{keyword}.png")
plt.show()
